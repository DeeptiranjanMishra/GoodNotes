...
import { BASE_URL_NOTE } from "../../link"
...
const Home = () => {
  ...
  const getAllNotes = async () => {
    try {
      const res = await axios.get(BASE_URL_NOTE+"/all", {
  ...
  const onSearchNote = async (query) => {
    try {
      const res = await axios.get(BASE_URL_NOTE+"/search", {
        params: { query },
        withCredentials: true,
      })
  ...
  const updateIsPinned = async (noteData) => {
    ...
    try {
      const res = await axios.put(
        BASE_URL_NOTE+"/update-note-pinned/" + noteId,
